...
#error 这个版本不准执行
        ...
        /// 整合所有核心指令，展示完整执行逻辑
        /// </summary>
        static void PreprocessorDirectiveDemo()
        {
            Console.WriteLine("===== 预处理器指令执行演示 =====");

            // 演示#region 代码折叠（本质是预处理器指令）
            #region 折叠演示区域
            Console.WriteLine("这是折叠区域内的代码，可通过#region折叠/展开");
            #endregion

            // 基于#if指令展示编译结果
            // 由于当前定义了Unity4，注释了#error，会执行对应逻辑
#if Unity4
            Console.WriteLine("\n当前编译分支：匹配Unity4符号");
#elif Unity2017
            Console.WriteLine("\n当前编译分支：匹配Unity2017符号");
#else
            Console.WriteLine("\n当前编译分支：匹配其它版本");
#endif

            // 手动触发警告（编译时会显示警告信息，不阻止编译）
#warning 这是自定义警告：演示#warning指令用法
            // 注释掉#error，避免编译失败
            // #error 这是自定义错误：编译时会直接终止编译
        }
    }
}